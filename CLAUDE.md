# CLAUDE.md — 樹配券 2.0 Product Manager Orchestrator
...
- 成敗回歸單一條件 `discount_amount > 0`；新券段失敗區分「點數端失敗」（treelife-api timeout；因點數系統目前無法查詢扣點結果與 tree/cub 拆分，暫時一律呼叫退點，原設計的「用點結果確認＋每日 cronjob 對帳」列為未來優化方向）與「我方失敗」（扣點成功但發券失敗，孤兒點數、人工善後），兩者皆不改變成敗判定
