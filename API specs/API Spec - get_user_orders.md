...
| items[].brand_id | String | 對應 brand 識別碼 |
| items[].brand_name | String | 對應 brand 名稱 |
| items[].cash_amount | Integer | 本次刷卡金額（元） |
| items[].card_last_four_digits | String | 該筆刷卡卡號後四碼，固定 4 碼數字字串 |
| items[].discount_amount | Integer | 本次實際折抵總金額（元） |
| items[].order_status | String | 訂單當前狀態：`PROCESSING` \| `COMPLETED` \| `CANCELLED` |
| items[].finalized_at | String \| null | 訂單最終化時間；`PROCESSING` 時為 `null` |
| items[].created_at | String | 訂單建立時間（UTC+8 ISO 8601） |

### 邏輯說明
- 列表為摘要資訊，不含 `coupons_used` 明細與 `events` 歷程；完整資訊請呼叫 `get_order`
- `items[].card_last_four_digits` 為建單時由發卡主機提供，供前台端在訂單列表顯示卡號辨識資訊
