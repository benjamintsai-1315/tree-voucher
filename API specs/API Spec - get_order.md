...
| coupons_used[].unit_discount_amount | Integer | 該券的折抵金額（元） |
...
| events[].event | String | 事件類型：`CREATED` \| `COMPLETED` \| `CANCELLED` |
| events[].occurred_at | String | 事件發生時間（UTC+8 ISO 8601） |
