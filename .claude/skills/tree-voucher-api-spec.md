...
每次使用此 skill 建立或修改 API spec 時，必須將 changelog 記錄寫入**目標 API spec 檔案**（例如 `API specs/API Spec - {api_name}.md`），不要寫在此 skill 檔案中。

## 寫入位置

在目標 API spec 檔案的**最頂端**（frontmatter 之後、`# API:` 標題之前）維護一個 changelog 表格。
...
| Date | Summary |
| ---- | ------- |
| YYYY-MM-DD | 本次變更的一行摘要 |
```

## 規則

- 每次修改該 spec 時，在表格**最上方**插入新一行（新的在前，舊的在後）。
- 不刪除既有紀錄。
- 若該 spec 是新建立的，第一筆記錄寫「初版建立」。
- Summary 用繁體中文，簡短描述本次做了什麼（更名、新增欄位、修改邏輯等）。
