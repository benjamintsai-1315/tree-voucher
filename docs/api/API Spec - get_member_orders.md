...
| point_used | Object | 本次訂單**新發券**消耗的點數總計；等於 `coupon_usage_summary[].coupon_usage.new_issued` 各筆的 `tree_points`/`cub_points` 加總（沿用既有券不消耗點數），見下表 |
...
- `campaign_name` 為 coupon 建立時凍結的快照值，不隨 campaign 事後改名回溯變動（見 PRD §二 Coupon 規則 1）。若 campaign 曾經改名，同一 `campaign_id` 底下不同批次發行的券可能對應不同的 `campaign_name`，會在 `coupon_usage_summary` 中各自成一筆（各自帶完整的 `coupon_usage.new_issued`/`existing`），不會合併也不會互相覆蓋——見 Sample 第 3 筆訂單範例：`campaign_id` 相同，但當前名稱「樹配券 $24」（本次新發券）與舊名稱「樹配券 $22」（沿用發行當下名稱的舊券）分屬兩筆
- `point_used` 與各筆 `coupon_usage.new_issued` 的 `tree_points`/`cub_points` 為同一份快照的不同呈現粒度：`point_used` = Σ 所有 campaign 的 `coupon_usage.new_issued.tree_points`/`cub_points`，非各自獨立來源，保留於訂單層級供前端不需自行迭代加總即可顯示點數總計
...
- 訂單取消時，`total_discount_amount` 與 `coupon_usage_summary[].coupon_usage.{new_issued|existing}.total_discount_amount` 皆維持既有欄位語意（前者歸零、後者為原始計算值），前端純依 `order_status = cancelled` 判斷顯示邏輯，不需額外欄位
