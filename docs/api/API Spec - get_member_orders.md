...
| total_discount_amount | Integer | 本次折抵總金額（元）；`order_status = processing` 時為清算當下計算之預計金額，`completed` 時為實際折抵金額，`cancelled` 時固定為 `0` |
| coupon_summary | Array | 本次訂單所用券，依 `campaign_id`+`campaign_name`+`is_new_issued` 三者組合聚合的使用摘要，見下表 |
| point_used | Object | 本次訂單**新發券**消耗的點數總計；等於 `coupon_summary[]` 中 `is_new_issued=true` 各筆的 `tree_points`/`cub_points` 加總（沿用既有券不消耗點數），見下表 |
...
> ⚠️ 本欄位與 `create_order` response 的 `coupon_summary` **同名但結構不同**：`create_order` 回應的是物件（`new_issued`／`existing` 兩組彙總，不分 campaign，供發卡主機對帳）；本 API 的 `coupon_summary` 是**陣列**（依 campaign 進一步拆分，供前台訂單列表顯示）。兩者粒度與型別皆不同，串接時請勿混淆。

| 欄位 | 類型 | 說明 |
| ---- | ---- | ---- |
| campaign_id | String | 該組券所屬 campaign 識別碼（ULID） |
| campaign_name | String | 該組券所屬 campaign 名稱（coupon 建立時的快照，不隨 campaign 事後改名回溯變動；若 campaign 曾經改名，同一 `campaign_id` 可能對應到不同的 `campaign_name`，見下方邏輯說明與 Sample 第 3 筆訂單範例） |
| is_new_issued | Boolean | `true`：本次訂單即時發行的新券；`false`：本次訂單使用的既有（舊）券 |
| quantity | Integer | 該組合使用的券張數 |
| total_discount_amount | Integer | 該組合小計折抵金額（元）；訂單取消（`order_status = cancelled`）時仍為原始計算值，前端應依 `order_status` 顯示為「已退回券匣」，不直接呈現金額 |
| tree_points | Integer | `is_new_issued=true`：本次消耗的小樹點(生活)；`is_new_issued=false`：該券**原始發行時**所使用的小樹點(生活)（歷史值，非本次消耗，與 `create_order` 的 `coupon_summary.existing.tree_points` 語意一致） |
| cub_points | Integer | 同上，小樹點(信用卡) |
...
### 邏輯說明
- 列表為摘要資訊，`coupon_summary` 為依 `campaign_id`+`campaign_name`+`is_new_issued` 分組的聚合統計，非逐張券明細；本 API 仍不含逐張 `coupons_used` 明細與 `events` 歷程，前台端不另提供單筆完整明細查詢
- `coupon_summary`／`point_used` 為 **`create_order` 建單當下即計算完成並寫入 order 記錄的快照**（詳見 `create_order.md`「`get_member_orders` 用券摘要快照」段落），本 API 直接讀取該快照回傳，不於查詢當下即時 JOIN／GROUP BY 聚合——藉此降低列表查詢的即時運算成本；快照寫入後不隨後續 coupon 狀態變化（如轉為 `settled`）而更動
- **只有實際被使用到的組合才輸出一筆 row**：某 campaign 若本次僅使用其中一類券（例如全部為既有券），僅輸出那一類的一筆，不會為未使用的類別（如本次沒有新發券）額外輸出全零 row
- 同一 campaign（同 `campaign_id`+`campaign_name`）若同時有新券與舊券於本次訂單被使用，依 `is_new_issued` 拆成**兩筆**回傳，不合併為一筆；前端如需該 campaign 這筆訂單的合計數字，需自行加總同 `campaign_id`+`campaign_name` 的多筆 row
- `campaign_name` 為 coupon 建立時凍結的快照值，不隨 campaign 事後改名回溯變動（見 PRD §二 Coupon 規則 1）。若 campaign 曾經改名，同一 `campaign_id` 底下不同批次發行的券可能對應不同的 `campaign_name`，會在 `coupon_summary` 中各自成一筆，不會合併也不會互相覆蓋——見 Sample 第 3 筆訂單範例：`campaign_id` 相同，但 `is_new_issued=true`（本次新發券，用當前名稱「樹配券 $24」）與 `is_new_issued=false`（舊券，沿用發行當下的舊名稱「樹配券 $22」）分屬兩筆
- `point_used` 與 `coupon_summary[]` 中 `is_new_issued=true` 各筆的 `tree_points`/`cub_points` 為同一份快照的不同呈現粒度：`point_used` = Σ 所有 `is_new_issued=true` 列的 `tree_points`/`cub_points`，非各自獨立來源，保留於訂單層級供前端不需自行迭代加總即可顯示點數總計
...
- 訂單取消時，`total_discount_amount` 與 `coupon_summary[].total_discount_amount` 皆維持既有欄位語意（前者歸零、後者為原始計算值），前端純依 `order_status = cancelled` 判斷顯示邏輯，不需額外欄位
