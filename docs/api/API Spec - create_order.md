...
- `coupon_discount_amount`（單張券折抵金額，供前端與 `campaign_name` 組合顯示）不另外快照，由同一筆聚合列計算而得：`(new_issued.total_discount_amount + existing.total_discount_amount) // (new_issued.quantity + existing.quantity)`（整數除法，無條件捨去）；因該值於 campaign 建立後即不可變更，同一 `campaign_id` 底下所有批次恆為同一值，不需額外儲存、也不列入分組鍵
